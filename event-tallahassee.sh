#!/usr/bin/env bash
#   They're fast, but stupid, just like twitch chat.
#
#   ./event-28seconds.sh
#   ./event-28seconds.sh --end
source "$(dirname "${BASH_SOURCE[0]}")/pz-event-lib.sh"

EVENT_NAME="Tallahassee or Colombus?"
EVENT_HOURS=""
EVENT_END_TIME="4"   # Ends at 4 AM
EVENT_BLURB="<RGB:1,0.2,0.2>Rule 2: Cardio.<LINE><RGB:0.4,1,0.4>All physical and combat skills gain 4x XP, but every. single. zed is a sprinter."
